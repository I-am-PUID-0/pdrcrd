...
then echo "Configuring the rclone mount name to $RCLONE_MOUNT_NAME";
else echo "Please set a name for the rclone mount" && exit 1;
fi
if [[ -n "$RD_API_KEY" ]];
then echo "Configuring the API Key";
